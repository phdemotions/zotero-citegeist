...
**Note from U1 review round A:** `verifyUpdateManifest`, now in `scripts/build-verify.mjs`, requires the `update.json` entry for the version being built to declare exactly `package.json`'s range. It cannot fail while `updateManifestFor` builds that entry from the same metadata. Once `update.json` is built from every line in `release-lines.json`, that equality holds only at a tag build, where the tagged version's entry must equal `package.json` (KTD3). Outside a tag build a line's published cap may exceed `package.json`'s, which is how a same-version cap raise works (AE6), but never fall below it. Call `verifyUpdateManifest` only in tag context and apply the never-below rule otherwise. The range readers in `scripts/build-metadata.mjs` (`rangeFromMetadata`, `rangeFromApplication`) normalise each layout to `{ min, max }`; the lines-file reader belongs beside them.
**Approach (KTD10):**
