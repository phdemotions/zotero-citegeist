...
**One writable choke point.** Every write gets its connection from `requireWritableDb` in `cache/db.ts`, the only function that hands out the `WritableDb` type every write helper takes. On a read-only cache it throws a `CacheWriteRefusedError` carrying `CG-DB03` or `CG-DB04`, and once `closeCache` has started, `CG-DB02`. The writers are `upsertRow`, `deleteRow`, `mutateRow` (behind `cacheWorkData`, the match writers and `clearPendingSuggestion`), `cacheItemAuthors`, `setCuratedItemAuthor`, `updateAuthorMetrics`, `reconcileAuthorMerge`, and the migration and orphan-GC deletes. A refused write rejects before it touches SQLite, the mirror or an item's `Extra` field, so no caller can mistake it for one that landed. `runQuery` holds the only `queryAsync` call, and `test/cache-write-invariants.test.ts` fails on a write issued without the choke point or a `queryAsync` call anywhere else.

Callers ask `cacheWriteRefusalCode()` before work that only matters if it can be saved. The fetch service returns a `cache-unwritable` result carrying the code before any OpenAlex request, and a batch stops at its first item, so a read-only session spends none of the user's budget. The item pane shows the code in place of Confirm and "Not this paper" rather than offering choices it can't keep. Startup shows one non-modal notice per launch, and every diagnostic report carries a `Cache: read-only` line that neither later failures nor Clear removes.

`query_only` is the backstop behind the choke point, and it belongs to the connection. Zotero 9.0.6 and later close and reopen a plugin database around their idle backup, so the cache re-applies it through `DBConnection.onConnect` for as long as the session is read-only. Zotero 10's idle `VACUUM INTO` of the plugin database then fails under `query_only` and logs an error to Zotero's console. That noise is accepted: the alternative is a connection that takes writes.
