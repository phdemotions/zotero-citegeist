...
- **General rule.** For any Zotero plugin-API registration (menus, columns, panes): the registry is process-global. Guard repeat registration with a module flag, and never tear down a process-global registration from a per-window unload handler. Element type dictates l10n form — menuitems are attribute-only (`.label`/`.accesskey`); pane headers/sidenav take bare values. Check the target element before assuming.
